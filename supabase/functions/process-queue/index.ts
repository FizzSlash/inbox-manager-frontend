...
serve(async (req)=>{
  ...
  try {
    ...
    // Get ALL available tasks for batch processing
    ...
    if (aiTasks.length > 0) {
      ...
      try {
        // Create batch requests with better duplicate handling
        const batchRequests = aiTasks.map((task, index)=>{
          // Generate unique custom_id
          let customId;
          if (task.lead_id && task.lead_id !== 'null' && task.lead_id !== null && !isNaN(parseInt(task.lead_id))) {
            customId = `lead_${task.lead_id}`;
          } else {
            customId = `task_${task.id}_${index}`; // Add index to ensure uniqueness
          }
          
          console.log(`🏷️ Task ${task.id}: lead_id="${task.lead_id}" (type: ${typeof task.lead_id}) -> custom_id="${customId}"`);
...
async function checkCompletedBatches(supabase) {
  try {
    ...
    const { data: processingBatches, error: batchError } = await supabase.from('ai_batches').select('*').eq('status', 'processing');
    if (batchError) {
        ...
        if (batchStatus.processing_status === 'ended') {
          ...
          if (resultsResponse.ok) {
            ...
            for (const result of results){
              ...
              // Handle both lead_ID and task_ID formats
              if (customId.startsWith('lead_')) {
                leadId = parseInt(customId.replace('lead_', ''));
              } else if (customId.startsWith('task_')) {
                // Extract task ID (handle both task_123 and task_123_0 formats)
                const taskIdMatch = customId.match(/task_(\d+)/);
                if (taskIdMatch) {
                  taskId = parseInt(taskIdMatch[1]);
                  // For task-based custom_ids, we'll need to look up the lead_id from the processing_queue
                  ...
                  if (taskData && taskData.lead_id && taskData.lead_id !== 'null' && taskData.lead_id !== null) {
                    leadId = parseInt(taskData.lead_id);
                  }
                }
              }
              
              let intentScore = null; // Default to null instead of 5
              if (result.result && result.result.type === 'succeeded') {
                ...
                console.log(`⚠️ Lead ${leadId}: AI failed - keeping intent as null`);
              }
              
              // Update lead with intent score (only if we have a valid leadId)
              if (leadId) {
            ...
            // 🔧 CRITICAL FIX: Mark all tasks in this batch as completed
            console.log(`✅ Marking ${batchRecord.task_ids.length} tasks as completed...`);
            for (const taskId of batchRecord.task_ids) {
              await supabase.from('processing_queue').update({
                status: 'completed'
              }).eq('id', taskId);
            }
            ...
            console.log(`✅ Batch ${batchRecord.batch_id} results processed, leads updated, and ${batchRecord.task_ids.length} tasks marked as completed`);
          }
        }
      } catch (error) {
...
async function processAIIntent(supabase, payload, leadId) {
  ...
  const { data, error } = await supabase.from('retention_harbor').update({
    intent: null, // Use null instead of 5 as default
    processed: true
  }).eq('id', leadId);

...
const InboxManager = ({ user, onSignOut, demoMode = false }) => {
  ...
  useEffect(() => {
    if (user?.id) {
      setUser(user.id, user.user_metadata?.brand_id);
      reportInfo('User session started', {
        userId: user.id,
        email: user.email,
        demoMode: demoMode
      });
  ...
  // Simple function to check if AI processing is active (no complex tracking)
  const fetchQueueStatus = async () => {
    // Simplified: Just check if there are any recent AI tasks queued
    // No complex batch tracking needed
  };
  
  // Persistent AI processing toast state with localStorage persistence
  const [persistentAIToast, setPersistentAIToast] = useState(() => {
    try {
      const saved = localStorage.getItem('persistentAIToast');
      return saved ? JSON.parse(saved) : null;
    } catch {
      return null;
    }
  }); // { id, totalLeads, processedLeads, startTime }
  
  // Persist AI toast to localStorage whenever it changes
  useEffect(() => {
    if (persistentAIToast) {
      localStorage.setItem('persistentAIToast', JSON.stringify(persistentAIToast));
    } else {
  ...
  }, [user?.id]); // Only run once when user changes, not on every state update
  ...
  // Immediate save for AI-generated drafts (no debounce) - includes Supabase save
  const saveDraftImmediate = async (leadId, content, htmlContent) => {
    ...
    try {
      const currentBrandId = user?.user_metadata?.brand_id || sessionStorage.getItem('user_brand_id');
      if (currentBrandId && content.trim()) {
        await supabase
    ...
    try {
      ...
      if (currentBrandId && user) {
        ...
        if (error) {
          ...
          // Don't immediately fail - try sessionStorage backup first
          console.log('🔄 Supabase failed, trying sessionStorage backup...');
          ...
          // Enhanced sessionStorage with timestamp and user validation
          const sessionData = {
            ...newState,
            timestamp: Date.now(),
            userId: user?.id,
          ...
          sessionStorage.setItem('apiKeys_session', JSON.stringify(sessionData));
      ...
      // STEP 3: Fallback to sessionStorage with validation
      console.log('📱 Trying sessionStorage...');
      const backup = sessionStorage.getItem('apiKeys_session');
      if (backup) {
        const parsed = JSON.parse(backup);
        
        // Validate session data is still fresh and for correct user
        const isValidSession = parsed.userId === user?.id && 
                             parsed.brandId === currentBrandId &&
                             (Date.now() - parsed.timestamp) < (2 * 60 * 60 * 1000); // 2 hours
        
        if (isValidSession) {
          // Remove metadata before setting state
          const { timestamp, userId, brandId, ...apiKeysData } = parsed;
          setApiKeys(apiKeysData);
          console.log('✅ Loaded from sessionStorage backup (validated)');
          return;
        } else {
          console.log('⚠️ SessionStorage data is stale or invalid, clearing...');
  ...
  // Clear session storage only when user actually changes (not on token refresh)
  useEffect(() => {
    const currentUserId = sessionStorage.getItem('cached_user_id');
    if (user?.id && currentUserId && currentUserId !== user.id) {
      // Only clear when switching between different users, not on token refresh
      sessionStorage.removeItem('apiKeys_session');
      sessionStorage.removeItem('user_brand_id');
      console.log('🧹 User changed - cleared sessionStorage for fresh session');
    }
  }, [user?.id]); // Only run when user ID changes

  // Keep session storage on unmount for better user experience
  // Only clear on actual logout, not component unmount



  // Periodic refresh for long-running sessions (every 30 minutes)
  useEffect(() => {
    if (!user?.id || demoMode) return;
    
    const refreshInterval = setInterval(() => {
      console.log('🔄 Periodic refresh - reloading API keys and brand data...');
      if (brandId && user?.id) {
        loadApiKeys();
      }
  ...
  }, [user?.id, brandId, demoMode]);

  // Load API keys when brandId and user are available
  useEffect(() => {
    // Small delay to ensure brandId state is properly set from cache
    ...
    }, 100); // 100ms delay to allow brandId state to settle
    
    return () => clearTimeout(timeoutId);
  }, [user?.id, brandId]); // Run when user ID OR brandId changes ONLY
  ...
  // Debounced fetchLeads to prevent rapid calls
  useEffect(() => {
    ...
    // Only fetch when ALL conditions are met (prevent race conditions)
    if (brandId && user?.id && !isLoadingApiKeys && apiKeys.accounts.length > 0) {
      console.log('📊 FETCH LEADS: ⚡ All conditions met - scheduling fetchLeads');
      
      // Debounce to prevent rapid calls
      const timeoutId = setTimeout(() => {
        console.log('📊 FETCH LEADS: Executing debounced fetchLeads');
        fetchLeads();
      }, 500); // 500ms delay
      
      return () => clearTimeout(timeoutId);
    } else {
      console.log('⏳ FETCH LEADS: Waiting for conditions:', {
        hasBrandId: !!brandId,
        hasUserId: !!user?.id,
        isLoadingApiKeys,
        apiKeyCount: apiKeys.accounts.length
      });
    }
  ...
  useEffect(() => {
    ...
    if (queueProcessorActive && user?.id) {
      ...
      //   processQueue();
      // }, 60000);
      console.log('✅ Queue processor: GitHub Actions handles processing every minute');
    }
  ...
  const pollForNewLeads = async (isManual = true) => {
    try {
      ...
      // Get SmartLead accounts
      const smartleadAccounts = apiKeys.accounts.filter(acc => 
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      // Early exit if conditions aren't met (prevents race conditions)
      if (!brandId || !user?.id || isLoadingApiKeys || apiKeys.accounts.length === 0) {
        console.log('❌ FETCH LEADS: Conditions not met - aborting early', {
          hasBrandId: !!brandId,
          hasUserId: !!user?.id,
          isLoadingApiKeys,
          apiKeyCount: apiKeys.accounts.length
      ...
      const transformedLeads = filteredData.map(lead => {
        ...
        // Note: Intent score comes from database (AI analysis), not calculated here
        return {
          id: lead.id,
      ...
      trackAPICall('fetchLeads', duration, true);
      reportInfo('Leads fetched successfully', { 
      ...
      // Track failed API call and report error
      const duration = Date.now() - startTime;
      trackAPICall('fetchLeads', duration, false);
      reportError(err, {
        function: 'fetchLeads',
        brandId: user?.user_metadata?.brand_id,
        duration: duration,
        demoMode: demoMode
      });
  ...
  // ===== MASTER INBOX API FUNCTIONS =====
  
  // Master Inbox Backfill - Complete version with pagination and database storage
  const backfillFromMasterInbox = async (apiKey, accountId, selectedConfig = null, daysPast = 30) => {
    console.log(`🚀 MASTER INBOX BACKFILL: Starting backfill for past ${daysPast} days...`);
    ...
    try {
      ...
      setBackfillProgress({ 
        ...
        status: `Fetching leads from past ${daysPast} days...` 
      });
      ...
      console.log(`📅 DATE FILTER: Fetching responses from ${startDate.toISOString()} to ${endDate.toISOString()}`);
      
      // Step 2: Fetch pages from Master Inbox API with date filtering and pagination
      let offset = 0;
      const limit = 20; // API maximum
      let hasMorePages = true;
      
      while (hasMorePages) {
        // Build filters object with date range
        const filters = {
          replyTimeBetween: [startDate.toISOString(), endDate.toISOString()]
        };
        
        // Add category filters if specified
        if (selectedConfig?.importCategories) {
          filters.leadCategories = {
            categoryIdsIn: selectedConfig.importCategories
          };
        }
        ...
        for (const lead of transformedBatch) {
          try {
            ...
            if (parsedConvo) {
              lead.parsed_convo = parsedConvo;
              validBatchLeads.push(lead);
            }
          } catch (error) {
            // Skip leads that can't be parsed
          }
        }
        
        // Add valid leads to our collection
        allLeads.push(...validBatchLeads);
        totalProcessed += validBatchLeads.length;
        
        // Update progress
        setBackfillProgress({ 
          current: totalProcessed, 
          total: totalProcessed + (pageLeads.length === limit ? limit : 0), // Estimate
          status: `Fetched ${totalProcessed} leads from past ${daysPast} days...` 
        });
        ...
        if (pageLeads.length < limit) {
          ...
          offset += limit;
          // Rate limit protection: 1.2 second delay (SmartLead allows 60 requests per minute)
          await new Promise(resolve => setTimeout(resolve, 1200));
        }
      }

      // === STEP 1 COMPLETE ===
      console.log(`📊 BACKFILL STEP 1: ${allLeads.length} leads fetched from API and parsed`);
      
      if (allLeads.length === 0) {
        throw new Error('No leads with parseable conversation data found');
      }
      
      // === STEP 2: UPSERT TO DATABASE ===
      console.log(`📊 BACKFILL STEP 2: Starting upsert of ${allLeads.length} leads to database...`);
      setBackfillProgress({ 
        current: 0, 
        total: allLeads.length, 
        status: `Upserting ${allLeads.length} leads to database...` 
      });

      let storedCount = 0;

      // Prepare all leads for upsert - only using columns that exist in the table
      const allLeadsForUpsert = allLeads.map((lead, index) => {
        // Debug: Check if email_message_body is valid before upserting
        const hasValidMessageBody = lead.email_message_body && lead.email_message_body !== '[]' && lead.email_message_body.length > 10;
        if (!hasValidMessageBody) {
          console.log(`⚠️ UPSERT: Lead ${lead.email} has invalid message body: "${lead.email_message_body?.substring(0, 50)}..."`);
        }
        
        // Debug: Log the first few leads to see what data we have
        if (index < 3) {
          console.log(`🔍 UPSERT DEBUG: Lead ${index + 1}:`, {
            email: lead.email,
            lead_email: lead.lead_email,
            first_name: lead.first_name,
            last_name: lead.last_name,
            ...
            id: lead.id,
            hasMessageBody: hasValidMessageBody
          });
        }
        ...
        return {
          ...
          lead_email: lead.email || lead.lead_email, // Use either field that has data
          first_name: lead.first_name,
          last_name: lead.last_name,
          campaign_ID: lead.campaign_id,
          lead_ID: lead.id,
          email_message_body: lead.email_message_body,
          parsed_convo: lead.parsed_convo ? JSON.stringify(lead.parsed_convo) : null, // ✅ OPTIMIZED: Use already-parsed conversation from batch processing
          lead_category: lead.lead_category?.toString() || '1',
          subject: lead.subject,
          website: lead.website || lead.email?.split('@')[1] || lead.lead_email?.split('@')[1] || '',
          phone: lead.phone || '',
          status: 'INBOX',
          intent: null, // Will be set by AI analysis
          opened: true, // Default to true based on table default
          campaign_name: lead.campaign_name || '',
          last_reply_time: lead.last_reply_time || null, // ✅ FIXED: Include last_reply_time from API
          // Note: created_at will be auto-set by database default (now())
          // Note: removed updated_at - column doesn't exist
        };
      });

      // Remove duplicates by email + brand + account to prevent "affect row a second time" error
      const uniqueLeads = [];
      const seenKeys = new Set();
      
      for (const lead of allLeadsForUpsert) {
        // Create unique key: email + brand + account
        const uniqueKey = `${lead.lead_email}|${lead.brand_id}|${lead.email_account_id}`;
      ...
      const duplicatesRemoved = allLeadsForUpsert.length - uniqueLeads.length;
      if (duplicatesRemoved > 0) {
        console.log(`📊 REMOVED ${duplicatesRemoved} duplicates from batch`);
      }

      // Use upsert with existing constraint (lead_email + brand_id)
      const { error: upsertError } = await supabase
        .from('retention_harbor')
        .upsert(uniqueLeads, {
          onConflict: 'lead_email,brand_id' // Handle conflicts on the actual unique constraint
        });
      ...
      if (upsertError) {
        ...
        storedCount = uniqueLeads.length;
        console.log(`📊 BACKFILL STEP 2 COMPLETE: ${storedCount} leads upserted to database`);
      }

              // === STEP 3: QUEUE FOR AI ANALYSIS ===
        if (storedCount > 0) {
          console.log(`📊 BACKFILL STEP 3: Queuing ${storedCount} leads for AI analysis...`);
          setBackfillProgress({ 
            current: storedCount, 
            total: allLeads.length, 
            status: `Queuing ${storedCount} leads for AI analysis...` 
          });

          try {
            // Fetch the actual records from Supabase to get their IDs
            const leadEmails = allLeads.slice(0, storedCount).map(lead => lead.email);
            const { data: supabaseLeads, error: fetchError } = await supabase
              .from('retention_harbor')
              .select('id, lead_email, email_message_body, brand_id, lead_category')
              .eq('brand_id', currentBrandId)
              .in('lead_email', leadEmails);

            if (fetchError) {
              throw new Error(`Failed to fetch upserted leads: ${fetchError.message}`);
            }
            ...
            // Show persistent AI toast if leads were queued
            if (analyzedCount > 0) {
              setPersistentAIToast({
                id: 'master_inbox_ai_' + Date.now(),
                totalLeads: analyzedCount,
                processedLeads: 0,
                startTime: new Date()
              });
            }
          } catch (aiError) {
            console.error('❌ AI ANALYSIS: Failed to queue leads:', aiError);
            // Don't fail the entire backfill if AI queueing fails
          }
        }

      // === BACKFILL SUMMARY ===
      console.log(`📊 BACKFILL COMPLETE:`);
      console.log(`   • ${allLeads.length} leads fetched from API`);
      console.log(`   • ${storedCount} leads upserted to database`);
      console.log(`   • All leads queued for AI analysis`);
        
        setBackfillProgress({ 
          current: storedCount, 
          total: allLeads.length, 
          status: `✅ Complete! ${storedCount} leads processed` 
        });

        showToast(`✅ Backfill complete! ${storedCount} leads from past ${daysPast} days processed.`, 'success');
      ...
      console.error('❌ MASTER INBOX BACKFILL: Failed:', error);
      setBackfillProgress(null);
      setIsBackfilling(false);
  ...
  // Fetch all replies from SmartLead Master Inbox API (simple version - first 20 only)
  const fetchMasterInboxReplies = async (apiKey) => {
    console.log('📥 MASTER INBOX: Fetching replies from Master Inbox API...');
    
    try {
      // Try with minimal parameters first to test basic connectivity
      console.log(`🔗 TESTING: Making request to Master Inbox API with key length: ${apiKey?.length}`);
      ...
      const response = await fetch(`https://server.smartlead.ai/api/v1/master-inbox/inbox-replies?api_key=${encodeURIComponent(apiKey)}&fetch_message_history=true`, {
        ...
        body: JSON.stringify({ 
          offset: 0,
          limit: 20,   // Maximum allowed by SmartLead API
          filters: {},
          sortBy: "REPLY_TIME_DESC"
        })
      });
      ...
      if (!response.ok) {
        ...
        try {
          errorBody = await response.text();
          console.error('🚨 TESTING: Error response body:', errorBody);
        } catch (e) {
          console.error('🚨 TESTING: Could not read error response body');
        }
        throw new Error(`Master Inbox API failed: ${response.status} - ${response.statusText}. Response: ${errorBody}`);
      }
      
      const result = await response.json();
      console.log(`✅ MASTER INBOX: Retrieved ${result?.data?.length || 0} leads with replies`);
      console.log('🔍 TESTING: Full API response structure:', result);
      
      return result;
      
    } catch (error) {
      console.error('❌ MASTER INBOX: Failed to fetch replies:', error);
      throw error;
    }
  };

  // Fetch ALL replies from SmartLead Master Inbox API with pagination (for polling)
  const fetchMasterInboxRepliesWithPagination = async (apiKey, sinceTimestamp = null) => {
    console.log('📥 MASTER INBOX POLLING: Fetching ALL recent replies with pagination...');
    ...
    try {
      ...
      let hasMorePages = true;
      let pageCount = 0;
      const maxPages = 10; // Safety limit: max 200 leads (10 pages × 20)
      
      // Build filters - only get leads since our cutoff time
      const filters = {};
      if (sinceTimestamp) {
        // Filter for replies after our cutoff time
        const cutoffDate = new Date(sinceTimestamp);
        filters.replyTimeBetween = [cutoffDate.toISOString(), new Date().toISOString()];
        console.log(`🔍 POLLING: Filtering for replies after ${cutoffDate.toISOString()}`);
      }
      
      while (hasMorePages && pageCount < maxPages) {
        pageCount++;
        console.log(`📥 POLLING: Fetching page ${pageCount} (offset: ${offset})`);
        ...
        console.log(`✅ POLLING: Page ${pageCount} retrieved ${pageLeads.length} leads`);
      ...
      console.log(`✅ POLLING: Retrieved total of ${allLeads.length} leads across ${pageCount} pages`);
      
      return { data: allLeads };
      
    } catch (error) {
      console.error('❌ POLLING: Failed to fetch replies with pagination:', error);
      throw error;
    }
  };

  // Transform Master Inbox data to match current lead structure
  const transformMasterInboxToLeads = (masterInboxData) => {
    console.log('🔄 MASTER INBOX: Transforming data to current lead format...');
    
    if (!masterInboxData?.data || !Array.isArray(masterInboxData.data)) {
      console.warn('⚠️ MASTER INBOX: Invalid data structure received');
      return [];
    }
    
    return masterInboxData.data.map(lead => {
      // Transform email history to match current conversation format with response time calculation
      const emailHistory = lead.email_history || [];
      
      // Debug: Check if this lead has email history
      if (!emailHistory || emailHistory.length === 0) {
        console.log(`⚠️ TRANSFORM: Lead ${lead.lead_email} has no email history`);
      } else {
        const replyCount = emailHistory.filter(msg => msg.type === 'REPLY').length;
        const sentCount = emailHistory.filter(msg => msg.type === 'SENT').length;
        console.log(`🔍 TRANSFORM: Lead ${lead.lead_email} - ${sentCount} SENT, ${replyCount} REPLY messages`);
      }
      
      const conversation = emailHistory.map((msg, index) => {
        // Calculate response time exactly like current system
        const prevMsg = emailHistory[index - 1];
        let responseTime = undefined;
        if (msg.type === 'REPLY' && prevMsg && prevMsg.type === 'SENT') {
          const timeDiff = new Date(msg.time) - new Date(prevMsg.time);
          responseTime = timeDiff / (1000 * 60 * 60); // Convert to hours
        ...
        return {
          ...
          opened: (msg.open_count || 0) > 0,
          clicked: (msg.click_count || 0) > 0,
          response_time: responseTime, // This was missing!
      ...
      return {
        // Core identification
        id: lead.email_lead_id || lead.email_lead_map_id || `master_${lead.lead_email}_${Date.now()}`,
        email: lead.lead_email,
        lead_email: lead.lead_email, // Keep both for compatibility
        ...
        // Conversation data (pre-built from Master Inbox!)
        conversation: conversation,
        email_message_body: JSON.stringify(conversation), // Store transformed conversation for fetchLeads compatibility
        
        // Timing data
        last_reply_time: lead.last_reply_time,
        last_sent_time: lead.last_sent_time,
        created_at_lead: lead.last_reply_time, // Use reply time as creation time
        
        // Status and categories (ensure compatibility with current system)
        lead_category: lead.lead_category_id || 1, // Keep as number for findCategoryOption
        lead_status: lead.lead_status || 'INPROGRESS',
        current_sequence_number: lead.current_sequence_number || 1,
        
        // SmartLead specific fields
        has_new_unread_email: lead.has_new_unread_email || false,
        ...
        subject: conversation.find(msg => msg.subject)?.subject || `Campaign: ${lead.email_campaign_name}`,
        website: '', // Will be filled from Supabase if available
        phone: '',   // Will be filled from Supabase if available
        
        // Mark as Master Inbox source for debugging
        data_source: 'master_inbox'
      };
    });
  };

  // Merge Master Inbox leads with Supabase custom data
  const mergeInboxWithSupabaseData = (masterInboxLeads, supabaseData, userAccountIds) => {
    console.log('🔗 MERGE: Starting data merge process...');
    console.log(`📊 MERGE: Master Inbox leads: ${masterInboxLeads.length}`);
    console.log(`📊 MERGE: Supabase records: ${supabaseData.length}`);
    console.log(`🔑 MERGE: User account IDs: ${userAccountIds.length}`);
    
    const mergedLeads = masterInboxLeads.map(masterLead => {
      // Find matching Supabase record using your current logic:
      // Match by email + email_account_id
      const supabaseMatch = supabaseData.find(s => {
        // Primary match: email + email_account_id
        if (s.lead_email === masterLead.email && s.email_account_id) {
          return userAccountIds.includes(s.email_account_id);
        }
        
        // Fallback match: email + brand_id (for backward compatibility)
        if (s.email === masterLead.email || s.lead_email === masterLead.email) {
          return userAccountIds.some(accountId => {
            // Check if this lead belongs to any of the user's accounts
            const account = apiKeys.accounts.find(acc => acc.account_id === accountId);
            return account && s.brand_id === account.brand_id;
          });
        }
      ...
      const mergedLead = {
        // Master Inbox data (conversation, timing, campaign info)
        ...masterLead,
        
        // Your custom fields from Supabase (source of truth)
        intent: supabaseMatch?.intent || null,
        notes: supabaseMatch?.notes || '',
        tags: supabaseMatch?.tags || [],
        is_read: supabaseMatch?.is_read || false,
        custom_field: supabaseMatch?.custom_field || '',
        deal_size: supabaseMatch?.deal_size || 0,
        call_booked: supabaseMatch?.call_booked || false,
        closed: supabaseMatch?.closed || false,
        website: supabaseMatch?.website || '',
        phone: supabaseMatch?.phone || '',
        
        // Preserve Supabase metadata
        supabase_id: supabaseMatch?.id || null,
        supabase_created_at: supabaseMatch?.created_at || null,
        
        // Merge flags for debugging
        has_supabase_data: !!supabaseMatch,
        data_source: 'master_inbox_merged'
      };
      
      // Log successful matches for debugging
      if (supabaseMatch) {
        console.log(`✅ MERGE: Matched ${masterLead.email} with Supabase record ${supabaseMatch.id}`);
      } else {
        console.log(`⚠️ MERGE: No Supabase match for ${masterLead.email} (new lead)`);
      }
      
      return mergedLead;
    });
    
    console.log(`🎯 MERGE: Successfully merged ${mergedLeads.length} leads`);
    console.log(`📈 MERGE: Supabase matches: ${mergedLeads.filter(l => l.has_supabase_data).length}`);
    console.log(`📝 MERGE: New leads: ${mergedLeads.filter(l => !l.has_supabase_data).length}`);
    
    return mergedLeads;
  };

  // ✅ REMOVED: fetchLeadsViaMasterInbox() - was unused
  /*
  const fetchLeadsViaMasterInbox = async () => {
    console.log('🚀 MASTER INBOX: Starting hybrid fetch process...');
    ...
    try {
      ...
      // Get brand ID (same as your current system)
      const currentBrandId = user?.user_metadata?.brand_id || sessionStorage.getItem('user_brand_id');
      if (!currentBrandId) {
        throw new Error('No brand ID found - user may need to complete signup');
      }
      ...
      if (smartleadAccounts.length === 0) {
        throw new Error('No SmartLead API keys configured');
      }
      
      // STEP 1: Fetch from Master Inbox API (fast - 2-3 seconds)
      console.log('📥 MASTER INBOX: Fetching conversation data...');
      const allInboxData = [];
      
      for (const account of smartleadAccounts) {
        const decryptedApiKey = decryptApiKey(account.esp.key);
        const inboxData = await fetchMasterInboxReplies(decryptedApiKey);
        const transformedLeads = transformMasterInboxToLeads(inboxData);
        allInboxData.push(...transformedLeads);
      }
      
      console.log(`✅ MASTER INBOX: Retrieved ${allInboxData.length} leads with conversations`);
      
      // STEP 2: Fetch custom data from Supabase (your current logic)
      console.log('📊 SUPABASE: Fetching custom fields...');
      ...
      console.log(`✅ SUPABASE: Retrieved ${supabaseData?.length || 0} custom records`);
      
      // STEP 3: Merge the data (best of both worlds)
      const userAccountIds = apiKeys.accounts.map(acc => acc.account_id);
      const mergedLeads = mergeInboxWithSupabaseData(allInboxData, supabaseData || [], userAccountIds);
      
      // STEP 4: Apply your existing transformation logic for UI compatibility
      const finalLeads = mergedLeads.map(lead => {
        // Calculate metrics exactly like your current system
        ...
        // Calculate engagement score like current system
        const engagementScore = sent.length > 0 ? Math.min((replies.length / sent.length) * 100, 100) : 0;
        
        // Create lead object matching your current system exactly
        return {
          // Core identification (match current system)
          id: lead.id,
          campaign_id: lead.campaign_id || null,
          campaign_ID: lead.campaign_id || null, // Keep both for compatibility
          campaign_name: lead.campaign_name || null,
          lead_id: lead.id || null,
          email_stats_id: conversation.find(msg => msg.stats_id)?.stats_id || null,
          
          // Timestamps
          created_at: lead.supabase_created_at || lead.created_at_lead,
          updated_at: lead.supabase_created_at || lead.created_at_lead,
          created_at_best: lead.supabase_created_at || lead.created_at_lead,
          
          // Contact info
          email: lead.email,
          lead_email: lead.email, // Keep both
          first_name: lead.first_name,
          last_name: lead.last_name,
          ...
          // Metrics and scores
          intent: lead.intent, // From Supabase (your custom scoring)
          response_time_avg: avgResponseTime,
          engagement_score: engagementScore,
          
          // Categories and tags (match current system)
          lead_category: lead.lead_category,
          tags: [leadCategoryMap[parseInt(lead.lead_category)] || 'Uncategorized'],
          
          // Conversation data
          conversation: conversation,
          
          // Status and flags
          opened: lead.is_read || false, // Map your is_read to opened
          status: lead.status || 'INBOX',
          ...
          // Debug fields
          has_supabase_data: lead.has_supabase_data,
          data_source: lead.data_source
        };
      });
      
      console.log(`🎉 MASTER INBOX: Complete! ${finalLeads.length} leads ready for display`);
      setLeads(finalLeads);
      
      // Sync positive response count (same as current system)
      await syncLeadCounts();
      
    } catch (error) {
      console.error('❌ MASTER INBOX: Hybrid fetch failed:', error);
  ...
  */
  ...
  // Test Master Inbox API to see actual response structure
  const testMasterInboxAPI = async () => {
    console.log('🧪 TESTING: Starting Master Inbox API test...');
    
    try {
      // Find SmartLead accounts
      const smartleadAccounts = apiKeys.accounts.filter(acc => 
        acc.esp.provider === 'smartlead' && acc.esp.key
      );

      if (smartleadAccounts.length === 0) {
        showToast('Please add a SmartLead API key first before testing', 'error');
        console.error('❌ TESTING: No SmartLead API keys found');
        return;
      }

      showToast('Testing Master Inbox API...', 'info');
      console.log(`🔍 TESTING: Found ${smartleadAccounts.length} SmartLead account(s) to test`);

      // Test with the first SmartLead account
      const testAccount = smartleadAccounts[0];
      console.log(`🔑 TESTING: Using account "${testAccount.name}" for testing`);

      // Decrypt the API key before using it
      const decryptedApiKey = decryptApiKey(testAccount.esp.key);
      console.log(`🔓 TESTING: API key decrypted, length: ${decryptedApiKey?.length || 0}`);

      // Call the Master Inbox API
      const result = await fetchMasterInboxReplies(decryptedApiKey);
      
      // Log detailed results for analysis
      console.log('📊 TESTING: Master Inbox API Response Analysis:');
      console.log('='.repeat(50));
      console.log(`Total leads with replies: ${result?.data?.length || 0}`);
      
      if (result?.data && result.data.length > 0) {
        const sampleLead = result.data[0];
        console.log('\n📝 TESTING: Sample lead structure:');
        console.log('Lead ID:', sampleLead.email_lead_id);
        console.log('Email:', sampleLead.lead_email);
        console.log('Name:', sampleLead.lead_first_name, sampleLead.lead_last_name);
        console.log('Campaign:', sampleLead.email_campaign_name);
        console.log('Category:', sampleLead.lead_category_id);
        console.log('Has unread:', sampleLead.has_new_unread_email);
        console.log('Last reply:', sampleLead.last_reply_time);
        console.log('Message history length:', sampleLead.email_history?.length || 0);
        
        if (sampleLead.email_history && sampleLead.email_history.length > 0) {
          console.log('\n📧 TESTING: Sample message structure:');
          const sampleMessage = sampleLead.email_history[0];
          console.log('Message type:', sampleMessage.type);
          console.log('From:', sampleMessage.from);
          console.log('To:', sampleMessage.to);
          console.log('Subject:', sampleMessage.subject);
          console.log('Time:', sampleMessage.time);
          console.log('Has email body:', !!sampleMessage.email_body);
          console.log('Stats ID:', sampleMessage.stats_id);
        }

        // Show unique campaigns
        const campaigns = [...new Set(result.data.map(lead => lead.email_campaign_name))];
        console.log('\n🎯 TESTING: Campaigns with replies:');
        campaigns.forEach(campaign => console.log(`  - ${campaign}`));

        // Show category distribution
        const categories = result.data.reduce((acc, lead) => {
          const catId = lead.lead_category_id || 'uncategorized';
          acc[catId] = (acc[catId] || 0) + 1;
          return acc;
        }, {});
        console.log('\n📊 TESTING: Category distribution:');
        Object.entries(categories).forEach(([catId, count]) => 
          console.log(`  Category ${catId}: ${count} leads`)
        );

        // Test the transformation function
        console.log('\n🔄 TESTING: Testing data transformation...');
        const transformedLeads = transformMasterInboxToLeads(result);
        console.log(`✅ TESTING: Successfully transformed ${transformedLeads.length} leads`);
        
        if (transformedLeads.length > 0) {
          const transformedSample = transformedLeads[0];
          console.log('\n📝 TESTING: Transformed lead structure:');
          console.log('ID:', transformedSample.id);
          console.log('Email:', transformedSample.email);
          console.log('Name:', transformedSample.first_name, transformedSample.last_name);
          console.log('Campaign:', transformedSample.campaign_name);
          console.log('Conversation length:', transformedSample.conversation?.length || 0);
          console.log('Subject:', transformedSample.subject);
          console.log('Data source:', transformedSample.data_source);
        }

        showToast(`✅ Master Inbox API test successful! Found ${result.data.length} leads with replies. Check console for details.`, 'success');
        
      } else {
        console.log('📭 TESTING: No leads with replies found');
        showToast('Master Inbox API connected successfully, but no leads with replies found', 'info');
      }

      console.log('='.repeat(50));
      console.log('✅ TESTING: Master Inbox API test completed successfully');

    } catch (error) {
      console.error('❌ TESTING: Master Inbox API test failed:', error);
      showToast(`Master Inbox API test failed: ${error.message}`, 'error');
    }
  };
  ...
  const callNavviiAIForDraftGeneration = async (prompt) => {
    try {
      ...
      reportError(error, {
        function: 'callNavviiAIForDraftGeneration',
      ...
      if (error.message.includes('CORS') || error.message.includes('429') || error.message.includes('fetch')) {
        ...
        reportWarning('CORS proxy issue in draft generation', {
    ...
    try {
      ...
      console.log(`🎯 Using ${leadApiKey.esp.provider} provider (Account: ${leadApiKey.name})`);
       ...
       const leadForSending = {
         ...
         // Ensure compatibility with new lead structure
         campaign_id: selectedLead.campaign_ID || selectedLead.campaign_id,
         lead_id: selectedLead.lead_ID || selectedLead.lead_id,
         email_stats_id: selectedLead.email_stats_id || selectedLead.lead_ID // Fallback to lead_ID
       };
...
ONLY RESPOND WITH THESE FIELDS and the answer/link . Only use the web search tool once.`;
  ...
  const enrichLeadData = async (lead) => {
    ...
    try {
      console.log('🤖 Enriching lead with Navvii AI for:', lead.first_name, lead.lead_email || lead.email);
  ...
  const parseConversationForIntent = (chatHistory) => {
    ...
    try {
      ...
      console.log('📋 Raw history data:', typeof chatHistory, chatHistory?.substring?.(0, 200));
      return null;
    }
    ...
    console.log(`🔍 Parsing conversation with ${conversation.length} messages`);
    const replyCount = conversation.filter(msg => msg.type === 'REPLY').length;
    const sentCount = conversation.filter(msg => msg.type === 'SENT').length;
    console.log(`📊 Message breakdown: ${sentCount} SENT, ${replyCount} REPLY`);
    
    // Debug: Show first message structure to understand format
    if (conversation.length > 0) {
      console.log(`🔍 First message structure:`, {
        type: conversation[0].type,
        hasEmailBody: !!conversation[0].email_body,
        hasContent: !!conversation[0].content,
        emailBodyLength: conversation[0].email_body?.length || 0,
        contentLength: conversation[0].content?.length || 0
      });
    }

    // ✅ Process all imported leads since API only imports leads with replies
    console.log(`✅ Processing conversation with ${replyCount} replies for AI analysis`);

    // Extract only essential data for intent scoring
    const conversationSummary = conversation.map((msg, index) => {
      ...
      // Debug: Show content extraction for REPLY messages
      if (msg.type === 'REPLY' && index < 3) {
        console.log(`🔍 REPLY ${index}: raw="${msg.email_body?.substring(0, 50)}..." -> clean="${cleanContent.substring(0, 50)}..."`);
      }
    ...
    console.log(`✅ Successfully parsed conversation: ${conversationSummary.length} messages, ${intentPayload.conversation_length} chars total`);
  ...
  // ✅ REMOVED: analyzeMissedLeads() - was unused  
  /*
    ...
    try {
      ...
      for (const lead of missedLeads) {
        try {
          ...
          // Use already-parsed conversation from database if available
          let parsedConvo;
          if (lead.parsed_convo) {
            try {
              parsedConvo = JSON.parse(lead.parsed_convo);
              console.log(`✅ Using pre-parsed conversation for ${lead.email}`);
            } catch (e) {
              console.log(`⚠️ Failed to parse stored conversation for ${lead.email}, re-parsing...`);
              parsedConvo = parseConversationForIntent(lead.email_message_body);
            }
          } else {
            // Fallback: parse from raw conversation if no stored version
            parsedConvo = parseConversationForIntent(lead.email_message_body);
          }
      ...
      try {
        ...
        if (leadRecord.parsed_convo) {
          try {
            ...
            console.log(`✅ Using pre-parsed conversation for ${leadRecord.lead_email}`);
          } catch (e) {
            console.log(`⚠️ Failed to parse stored conversation for ${leadRecord.lead_email}, re-parsing...`);
...
${JSON.stringify(parsedConvo)}`;
    ...
    console.log(`   - ${analyzed} leads queued for AI analysis`);
  ...
  */
  ...
  const confirmCampaignSelection = async () => {
    ...
    // Start backfill with selected configuration using NEW Master Inbox API
    backfillFromMasterInbox(
  ...
  return (
    <>
                ...
                <button
                  ...
                  onClick={async (e) => {
                    ...
                    try {
                      ...
                      const decryptedApiKey = decryptApiKey(account.esp.key);
                      const categories = await fetchSmartleadCategories(decryptedApiKey);
                      setAvailableCategories(categories);
                      // Auto-select suggested categories
                      const suggestedIds = categories.filter(cat => cat.suggested).map(cat => cat.id);
                      setSelectedCategories(suggestedIds);
                    } catch (error) {
                    ...
                    // Reset backfill days to default
                    const defaultDays = 30;
                    setBackfillDays(defaultDays);
                    
                    // Show the Master Inbox backfill modal
                    setShowBackfillModal(true);
                  }}
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col-reverse gap-2">
        ...
        {/* AI Processing Status - Simple Version */}
        {persistentAIToast && (
          <div 
            className="flex flex-col gap-3 px-6 py-4 rounded-xl shadow-2xl min-w-[450px] border-2"
            style={{
              backgroundColor: themeStyles.primaryBg,
              border: `2px solid ${themeStyles.accent}`,
              backdropFilter: 'blur(16px)',
              boxShadow: `0 8px 32px ${themeStyles.accent}20`
            }}
          >
            {/* Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-semibold text-sm" style={{ color: themeStyles.textPrimary }}>
                    🤖 AI Intent Analysis
                  </div>
                  <div className="text-xs" style={{ color: themeStyles.textMuted }}>
                    Processing {persistentAIToast.totalLeads} leads in background
                  </div>
                </div>
              </div>
              ...
              {!isBackfilling ? (
                <>
                  <p className="text-sm mb-4 transition-colors duration-300" style={{color: themeStyles.textMuted}}>
                    Import leads who replied from SmartLead's Master Inbox. This uses the new optimized API to fetch all conversations across campaigns.
                  </p>
                  ...
                  <div className="mb-4 p-3 rounded-lg transition-colors duration-300" style={{backgroundColor: themeStyles.secondaryBg}}>
                    ...
                    {apiKeys.accounts.length === 0 ? (
                      ...
                      apiKeys.accounts
                        .filter(acc => acc.esp.provider === 'smartlead' && acc.esp.key)
                        .map(account => (
                  ...
                  <div className="mb-4" data-tour="timeframe-selector">
                    <label className="text-sm font-medium mb-3 block transition-colors duration-300" style={{color: themeStyles.textPrimary}}>
                      📅 Select Time Period to Import:
                    </label>
                    
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { days: 5, label: '5 Days', desc: 'Recent activity' },
                        { days: 10, label: '10 Days', desc: 'Past week+' },
                        { days: 30, label: '30 Days', desc: 'Last month' },
                        { days: 45, label: '45 Days', desc: '6+ weeks' },
                        ...
                        <button
                          ...
                          className={`p-3 rounded-lg text-left transition-all border ${backfillDays === days ? 'ring-2' : ''}`}
                          style={{
                            backgroundColor: backfillDays === days ? `${themeStyles.accent}15` : themeStyles.secondaryBg,
                            border: `1px solid ${backfillDays === days ? themeStyles.accent : themeStyles.border}`,
                            '--tw-ring-color': themeStyles.accent
                          }}
                        >
                    ...
                    <p className="text-xs mt-2 transition-colors duration-300" style={{color: themeStyles.textMuted}}>
                      ✨ Only imports leads who replied (Master Inbox optimization)
                    </p>
                  </div>
                  ...
                  <div className="flex justify-end gap-3">
                    ...
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        if (smartleadAccounts.length === 0) {
                          showToast('Please add a SmartLead API key first', 'error');
                          return;
                        }
                        ...
                        // Close modal and start Master Inbox backfill
                        setShowBackfillModal(false);
                        
                        try {
                          await backfillFromMasterInbox(decryptedApiKey, account.account_id, null, backfillDays);
                        } catch (error) {
                          console.error('❌ Master Inbox backfill failed:', error);
                          showToast(`Backfill failed: ${error.message}`, 'error');
                        }
                    ...
                    >
                      <Database className="w-4 h-4" />
                      {isBackfilling ? 'Starting Backfill...' : `Start ${backfillDays}-Day Backfill`}
                    </button>
                  </div>
                </>
              ) : (
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      backfillFromMasterInbox(
                        ...
                        30 // Default to 30 days for resumed backfill
                      ).then(() => {
